...
@interface ZGVideoFilterNV12Demo()

@property (atomic) int pendingCount;

@end

@implementation ZGVideoFilterNV12Demo {
...
- (ZegoVideoBufferType)supportBufferType {
    // * 返回滤镜的类型：此滤镜为异步 I420 滤镜
    return ZegoVideoBufferTypeAsyncNV12PixelBuffer;
}
...
- (CVPixelBufferRef)dequeueInputBuffer:(int)width height:(int)height stride:(int)stride {
    ...
    if (width_ != width || height_ != height || stride_ != stride) {
        ...
        if ([ZGImageUtils createNV12PixelBufferPool:&pool_ width:width height:height]) {
            width_ = width;
            height_ = height;
            stride_ = stride;
        } else {
