...
- (CVPixelBufferRef)shortVideoRecorder:(PLShortVideoRecorder *__nonnull)recorder cameraSourceDidGetPixelBuffer:(CVPixelBufferRef __nonnull)pixelBuffer timingInfo:(CMSampleTimingInfo)timingInfo {
    // 进行滤镜处理
    if (self.isPanning) {
...
//    }
    // 陌陌美颜处理
    NSError *error;
    CVPixelBufferRef pixelBuffer2 = [self.render renderPixelBuffer:pixelBuffer error:&error];
    if (error == nil) {
        pixelBuffer = pixelBuffer2;
    } else {
        NSLog(@"MMBeauty Process Error:%@",error);
    }
